...
define(function(require, exports, module){
	...
	require('http://api.map.baidu.com/getscript?v=1.4&ak=&services=&t=20140320104737');
	...
	(function () {
		...
		var YunlaiMap = function ($item, options) {
			...
			this._$mapRoutePanel = $('<div class="mapRoutePanel">'+
										'<div id="routeResult_'+ mapID +'" class="routeResult"><p class="noRouteInfo">暂无路线信息！</p></div>'+
		...
		YunlaiMap.prototype.addMarker = function(_marker) {
			...
			marker.addEventListener( 'click', function (e) {
				...
				//注册路线绘制按钮点击事件
				if(theClass._baiduMap.infoWindow && !window._yunlaiMap_initRouted){
					window._yunlaiMap_initRouted = true;
			...
			if(!fromPoint){
				...
				if(!theClass._transitRoute){
					theClass._transitRoute = new BMap.TransitRoute(theClass._baiduMap, {
						renderOptions: {map: theClass._baiduMap, panel: 'routeResult_1', autoViewport: true },
